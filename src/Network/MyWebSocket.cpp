...
{
    // 设置服务器的IP地址和端口
    std::string host = CManager.configVariable("REPORTUP_MESSAGE_IP");
    std::string port = CManager.configVariable("REPORTUP_MESSAGE_PORT");
    ...
    try
    {
        ...
        while (true)
        {
            ...
            {
                // 加入消息队列
                originalMessageQueue.push_queue(message);
            }
            else
            {
                // cout << "过滤消息：" << message << std::endl;
#ifdef DEBUG
                LOG_WARNING("过滤消息");
#endif
            }
